function [mu, KPost, Mus, KPosts, combinedXFuncH, combinedZFuncH, funcHs, ...
    sigmaSmOpts, sigmaPrOpts, A, learnedDecomp, margLikeVal] = ...
    addGPDecompMargLikelihood(X, y, Xtest, decomp, hyperParams)
...
if ~isfield(hyperParams, 'numBwSigmaDiRectIters') | isempty(hyperParams.numBwSigmaDiRectIters)
    numBwSigmaDiRectIters = 20;
else
    numBwSigmaDiRectIters = hyperParams.numBwSigmaDiRectIters;
end
...
nlmlF = @(t) normRotMargLikelihood( exp(t(1))*oneVec, exp(t(2))*oneVec, ...
    decomposition, A, X, y, meanFuncs, commonMeanFunc, noises, commonNoise);
[margLikeVal, optParams] = diRectWrap(nlmlF, diRectBounds, diRectOptions);
sigmaSmOpts = exp(optParams(1)) * oneVec;
sigmaPrOpts = exp(optParams(2)) * oneVec;
